...
import { findSession } from '../registry';
...
export const raceLocatorCommand: CliCommand = {
  ...
  async execute(args: string[]): Promise<void> {
    ...
    if (result.selectors.length === 0) {
      console.error('Usage: extended-playwright-cli race-locator <selector1> <selector2> ... [--timeout ms] [--visibility mode] [--session name]');
      process.exit(1);
    }
    ...
    try {
      ...
      const entry = await findSession(clientInfo, result.session);

      if (!entry) {
        console.error(`Race locator failed: No session '${result.session}' found. Run "extended-playwright-cli open" first.`);
        process.exit(1);
      }

      const session = new PlaywrightSession(clientInfo, entry.config);
      const res = await session.run({ _: ['run-code', script] });

      if (res && !res.isError && res.text) {
